//
//  NacsportXMLProjectImporter.swift
//  Youchip-Stat
//
//  Parses Nacsport-style XML (ALL_INSTANCES + optional ROWS) into ProjectImportModel.
//
//  Unlike the Sportcode importer, Nacsport puts the player (or team) in `code`
//  and every action/event of the clip in its own nested <label><text>. A single
//  instance therefore carries *several* labels, all of which are attached to the
//  resulting stamp as real labels (not collapsed into a single note).
//

import Foundation

enum NacsportXMLImportError: LocalizedError {
    case noInstances
    case parseFailed
...
private final class NacsportMutableInstance {
    var start: Double = 0
    var end: Double = 0
    var code: String = ""
    /// Every <label><text> of the instance, in document order.
    var labelTexts: [String] = []
}

private final class NacsportXMLParserDelegate: NSObject, XMLParserDelegate {
    struct RowColor {
        var code: String = ""
        var r: Int = 0
        var g: Int = 0
    ...
    private var instances: [(start: Double, end: Double, code: String, labels: [String])] = []
    ...
    private var currentInstance: NacsportMutableInstance?
    private var currentRow = RowColor()

    func resultInstances() -> [(start: Double, end: Double, code: String, labels: [String])] { instances }
    ...
    func parser(_ parser: XMLParser, didStartElement elementName: String, namespaceURI: String?, qualifiedName qName: String?, attributes attributeDict: [String: String] = [:]) {
        ...
        case "instance":
            inInstance = true
            inLabel = false
            currentInstance = NacsportMutableInstance()
        case "label":
    ...
    func parser(_ parser: XMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?) {
        ...
        case "instance":
            if let m = currentInstance {
                ...
                let cleanLabels = m.labelTexts
                    .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
                    .filter { !$0.isEmpty }
                // Fall back to the first label as the row identity when code is missing.
                let effectiveCode = codeTrim.isEmpty ? (cleanLabels.first ?? "") : codeTrim
                if !effectiveCode.isEmpty, m.end >= m.start {
                    instances.append((m.start, m.end, effectiveCode, cleanLabels))
                }
            }
            currentInstance = nil
            inInstance = false
        ...
        case "text":
            if inInstance, inLabel, !text.isEmpty {
                currentInstance?.labelTexts.append(text)
            }
        case "R":
...
enum NacsportXMLProjectImporter {

    private static let defaultImportedTagColor = "4A90D9"

    /// `XMLParser` expects UTF-8. Some tools save XML as UTF-16 (with or without BOM).
    ...
    static func makeProjectImport(from xmlData: Data, fileName: String) throws -> ProjectImportModel {
        ...
        // --- Build tags from `code` (player / team) values ---
        ...
        // --- Collect unique label texts (actions/events) across all instances ---
        var uniqueLabelTexts: [String] = []
        var seenLabels = Set<String>()
        for inst in rawInstances {
            for text in inst.labels where seenLabels.insert(text).inserted {
                uniqueLabelTexts.append(text)
            }
        }
        ...
        for text in uniqueLabelTexts {
            let lid = UUID().uuidString
            labelTextToId[text] = lid
            labels.append(Label(id: lid, name: text, description: ""))
        ...
        // --- Build one tag per code, all sharing the single label group ---
        ...
        for code in uniqueCodes {
            ...
            let tag = Tag(
                ...
                lablesGroup: labelGroupId.map { [$0] } ?? [],
                hotkey: nil,
                labelHotkeys: nil,
                mapEnabled: false,
                isInterval: true
        ...
        func stampForInstance(_ inst: (start: Double, end: Double, code: String, labels: [String])) -> TimelineStamp {
            let tag = tagByCode[inst.code]!
            var labelStructs: [FullLabelWithGroup] = []
            if let lgId = labelGroupId {
                for text in inst.labels {
                    if let lid = labelTextToId[text] {
                        labelStructs.append(FullLabelWithGroup(id: lid, name: text, description: "", lableGroupId: lgId))
                    }
                }
            }
        ...
        let customCollection = CustomCollectionExport(
            ...
            tags: uniqueCodes.map { tagByCode[$0]! },
            tagGroups: [tagGroup],
