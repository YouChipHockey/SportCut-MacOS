...
struct ExportWatermarkOptions {
    ...
    /// Наносить ли счётчики. Выбора «каких именно» нет намеренно: в файл идёт ровно то, что
    /// видно на экране в этот момент, — все записи, пересекающие кусок исходника. Показания
    /// берутся из записи разметки, поэтому счётчик идёт ровно как шёл вживую.
    var showClocks: Bool = false
