...
## SwiftUI-анимации
- `.transition(.scale/.move)` вешай ВНУТРИ `.offset`/`.position`, а не снаружи: они двигают
  только отрисовку, а layout-фрейм (от него считается якорь транзишена) остаётся на месте —
  иначе вью «вылетает» из угла контейнера.
